...
int store(FILE *dfp, int *addr, int *dedup, void*data, int storeing){// store data, if dedup -> dedup+1(same hash, same data), 
//storeing = 1 means it is pointer data(indirect, doubledirect) 
	...
	for(int j = 0; j < 1024*128; j++){
		for(int k = 7; k >= 0; k--){
			if((data_table[j]&(1<<k))!=0){
				...
				fseek(dfp,hstbid+dnum*20+4,SEEK_SET); // load number of link and hash(if bmap data == 1)
				...
				if(strcmp(md5hash,md5nodehash) == 0){ // if same hash
					...
					if(strcmp(tempdata2,tempdata1) == 0){ // compare data, if same
						...
						*addr = datbmid + dnum*4096;
						return 0; // hashnode link += 1, store address of data
					}
				}
			}
		}
	...
	for(int j = 0; j < 1024*128; j++){ // not same hash
		for(int k = 7; k >= 0; k--){
			if((data_table[j]&(1<<k))==0){
				...
				fseek(dfp,hstbid+dnum*20,SEEK_SET);
				fwrite(&linked,sizeof(int),1,dfp);
				fseek(dfp,hstbid+dnum*20 + 4,SEEK_SET);
				...
				*addr = datbmid + dnum*4096;
				...
				fwrite(data,sizeof(char),4096,dfp); // store data in data bm == 0, bm = 1, store(link = 1 ,hash)
				return 0;
			}
		}
	}
...
int DGFS_add(char* filename)
{
	...
	for(int i = 0; i <= (fsize-1)/(1024*4); i++){
		...
		if(i == (fsize-1)/(1024*4)){
			store(dfp, &addr, &dedup, data,0); // store data
		}
		else{store(dfp, &addr, &dedup, data,0);}
		if(i < 10){
			point[i] = addr;
		}else if(i < 10 + 1024){
			indpoint[i-10] = addr;
	...
	if((fsize-1)/(1024*4)>=10){
		store(dfp, &(point[10]), &deduptemp, indpoint,1); // store indirect pointer
	}
	if((fsize-1)/(1024*4) >= 1034){
		for(int i = 0; i <= ((fsize-1)/(1024*4) - 1034)/1024; i++){

			store(dfp, &(dobpointaddr[i]), &deduptemp, dobpoint[i],1); // store (deirected)indirect pointer
		}
		store(dfp, &(point[11]), &deduptemp, dobpointaddr,1); // store (double)indirect pointer
	}
